import {
    addTask,
    getTasks,
    markAsCompleted,
    updateTask,
    deleteTask,
    getUnassignedTasks,
    getTaskById,
    getTaskByProjectId
} from "./taskManager.js";

import { createTask } from "./taskModel.js";
import { archiveProject } from "../projects/projectManager.js";


export function createTaskController(taskPersistence) {

    // write operations

    // handle addition of tasks
    function handleAddTask(taskData) {

        // create a task
        const task = createTask(
            taskData.taskId,
            taskData.title,
            taskData.description,
            taskData.priority,
            taskData.projectId
        );

        // task creation failed
        if (!task) return false;

        // addition of task failed
        if (!addTask(task)) return false;

        // save current task collection
        taskPersistence.save(getTasks());

        // successful creation
        return true;
    }


    // handle completion of tasks
    function handleCompleteTask(taskId) {

        // get task before changing state
        const task = getTaskById(taskId);

        // task does not exist
        if (!task) return false;

        // mark task as complete
        const success = markAsCompleted(taskId);

        // completion failed
        if (!success) return false;

        // persist current state
        taskPersistence.save(getTasks());

        // archive project if applicable
        if (task.projectId) {
            archiveProject(task.projectId);
        }

        return true;
    }


    // handle updating of tasks
    function handleUpdateTask(taskId, updates) {

        // call update task function
        const success = updateTask(taskId, updates);

        // update failed
        if (!success) return false;

        // persist current state
        taskPersistence.save(getTasks());

        // successful update
        return true;
    }


    // handle deletion of tasks
    function handleDeleteTask(taskId) {

        // call delete task function
        const success = deleteTask(taskId);

        // deletion failed
        if (!success) return false;

        // persist new state after deletion
        taskPersistence.save(getTasks());

        // successful deletion
        return true;
    }


    // read operations

    // handle listing of tasks
    function handleListTasks() {
        return getTasks();
    }


    // handle listing of unassigned tasks
    function handleGetUnassignedTasks() {
        return getUnassignedTasks();
    }


    // handle fetching of a task by ID
    function handleGetTaskById(taskId) {
        return getTaskById(taskId);
    }


    // handle fetching tasks by project ID
    function handleGetTaskByProjectId(projectId) {
        return getTaskByProjectId(projectId);
    }


    return {
        handleAddTask,
        handleCompleteTask,
        handleUpdateTask,
        handleDeleteTask,
        handleListTasks,
        handleGetUnassignedTasks,
        handleGetTaskById,
        handleGetTaskByProjectId
    };
}